...
const calculateTime = (totalSecond) => {
    ...
    const updatingSecond = second < 10 ? `0${second}` : `${second}`;
    const result = `${minute}:${updatingSecond}`;
    return result;
}
...
audio.addEventListener("ended", () => {
    if(player.repeatMode === "one") {
        ...
        if(player.index + 1 < player.musicList.length) {
            nextMusic();
        }
    }
});
...
repeatBtn.addEventListener("click", () => {
    if (player.repeatMode === "all") {
        ...
        repeatIcon.textContent = "1";
    } 
    else if (player.repeatMode === "one") {
        player.repeatMode = "none";
        repeatIcon.className = "fa-solid fa-xmark";
    ...
    for(let i=0; i < list.length; i++) {
        let liTag = `
            <li li-index='${i}' onclick="selectedMusic(this)" class="list-group-item d-flex justify-content-between align-items-center">
                ...
                <span id="music-${i}" class="badge mx-3 bg-primary rounded-pill"></span>
